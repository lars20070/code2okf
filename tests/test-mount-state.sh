...
HELPER="${ROOT}/kits/code2okf/files/home/.local/lib/code2okf/mount-state.sh"
TEST_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/code2okf-state-test.XXXXXX")"
TESTS=0
...
skip() {
	[[ -z "${CODE2OKF_REQUIRE_BIND:-}" ]] ||
		fail "$* — CODE2OKF_REQUIRE_BIND is set"
	echo "skip - $*"
}
...
fresh() {
	...
	export CODE2OKF_STATE_DIR="${STATE}"
}
...
# relocation failure.
SPEC="${ROOT}/kits/code2okf/spec.yaml"
ENTRYPOINT="$(awk '
	$0 == "    - |" { block = 1; next }
	block && /^    - / { exit }
	block { sub(/^      /, ""); print }
...
mkdir -p "${FAKE_HOME}/.local/lib/code2okf" "${FAKE_BIN}"
# shellcheck disable=SC2016 # variables belong to the generated scripts
printf '#!/bin/sh\nexit "${MOUNT_STATE_TEST_STATUS}"\n' \
	>"${FAKE_HOME}/.local/lib/code2okf/mount-state.sh"
# shellcheck disable=SC2016 # variables belong to the generated scripts
printf '#!/bin/sh\nprintf "started\\n" >>"${ENTRYPOINT_AGENT_LOG}"\n' \
	>"${FAKE_BIN}/pi"
chmod +x "${FAKE_HOME}/.local/lib/code2okf/mount-state.sh" "${FAKE_BIN}/pi"
: >"${AGENT_LOG}"
MOUNT_STATE_TEST_STATUS=0 ENTRYPOINT_AGENT_LOG="${AGENT_LOG}" HOME="${FAKE_HOME}" \
	PATH="${FAKE_BIN}:${PATH}" sh -c "${ENTRYPOINT}" code2okf-entrypoint
...
calls="$(grep -c 'lib/code2okf/mount-state\.sh' "${SPEC}" || true)"
...
STARTUP="$(awk '
	...
	block { print }
' "${SPEC}")"
[[ "${STARTUP}" == *'lib/code2okf/mount-state.sh'* ]] ||
