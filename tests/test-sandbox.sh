...
# Check that the sandbox delivers what kits/code2okf/spec.yaml promises: the
# installed toolchain, the agent config copied in from kits/code2okf/files/, a
...
#
# Reuses the existing code2okf sandbox, creating one only if missing (minutes), so
# it may be testing a sandbox older than your last kits/code2okf/ edit. To force
# a fresh one: sbx rm --force code2okf && ./tests/test-sandbox.sh

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "${repo_root}"

kit_name="code2okf" # keyed to `name:` in kits/code2okf/spec.yaml and to sbx secrets
...
uv run python -m code2okf.sandbox
...
# pipe that never ends -- the same reason code2okf.sandbox runs every
