...
An installed code2okf has no checkout to fall back on: resources.py looks for
...
# What an installed code2okf needs in order to work with no checkout: the driver
...
_REQUIRED = (
    ...
    "code2okf/kit/files/home/.pi/agent/AGENTS.md",
    "code2okf/kit/files/home/.local/lib/code2okf/mount-state.sh",
    "code2okf/SPEC.md",
    "code2okf/clis/merkleokf/pyproject.toml",
    "code2okf/clis/merkleokf/src/merkleokf/merkle.py",
)
...
def test_wheel_still_carries_the_python_modules_beside_the_assets(wheel_names):
    ...
    hatchling currently finds src/code2okf by name even with force-include set,
    ...
    """
    modules = [name for name in wheel_names if name.startswith("code2okf/") and name.endswith(".py")]
    assert "code2okf/__init__.py" in modules
...
def test_wheel_excludes_local_state_and_test_trees(wheel_names):
    ...
    assert not [name for name in wheel_names if name.startswith("code2okf/clis/") and "/tests/" in name]
