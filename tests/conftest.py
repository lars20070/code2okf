...
`code2okf.sandbox` is the only module that reaches the outside world, so faking
...
from code2okf import sandbox as sandbox_module
from code2okf import workbench as workbench_module
...
def fake_sbx(monkeypatch: pytest.MonkeyPatch) -> FakeSbx:
    """Patch the seam code2okf.sandbox reaches the outside world through.
...
def isolated_state(tmp_path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(workbench_module, "LOCK_PATH_TEMPLATE", str(tmp_path / "code2okf-{uid}.lock"))
    return tmp_path
