"""Tests for code2okf.cli -- the end-to-end command, against FakeSbx."""
...
from code2okf import __version__, cli, sandbox, workbench
...
def test_version(capsys):
    ...
    assert capsys.readouterr().out.strip() == f"code2okf {__version__}"
...
def test_dry_run_does_nothing_paid_and_creates_nothing(tmp_path, capsys, isolated_state, fake_sbx):
    ...
    assert not (isolated_state / "xdg-state" / "code2okf").exists()
...
def test_dry_run_prints_the_command_lines_that_would_run(tmp_path, capsys, isolated_state, fake_sbx):
    ...
    assert "sbx run --detached --name code2okf" in out
    assert "sbx exec code2okf -- pi --mode json" in out
