...
# Runs INSIDE the code2okf sandbox, piped into `sbx exec ... sh -l -s` by
...
# The tool list must match BOTH lists in kits/code2okf/spec.yaml: the
...
# apt (kits/code2okf/spec.yaml). Ubuntu's `fd-find` package provides `fdfind`;
...
# npm through the retry wrapper (kits/code2okf/spec.yaml).
check pi

# npm, Markdown and spelling linters (kits/code2okf/spec.yaml).
check markdownlint-cli2
check cspell

# uv (kits/code2okf/spec.yaml).
check ruff
check yamllint

# setup.files shims (kits/code2okf/spec.yaml): workspace-backed CLIs on PATH.
check inspectmd
check inspectokf
check sizeokf
check merkleokf

# pinned release binaries (kits/code2okf/spec.yaml): checksummed download, no
# package manager.
check mq
check okfctl

# Config delivery: kits/code2okf/files/home/.pi/agent/ is copied at kit build
...
check_file "${HOME}/.local/lib/code2okf/mount-state.sh"
...
if [ -z "${CODE2OKF_STATE_DIR:-}" ]; then
	echo "MISSING CODE2OKF_STATE_DIR"
	failures=$((failures + 1))
else
	session_link="${HOME}/.pi/agent/sessions"
	session_target="${CODE2OKF_STATE_DIR}/sessions"
...
# of everything below it. sudo is passwordless here (kits/code2okf/spec.yaml), so
...
for rw in "${wiki_root}" "${CODE2OKF_STATE_DIR:-/nonexistent}/sessions"; do
...
if [ -n "${CODE2OKF_STATE_DIR:-}" ]; then
	for host_only in sandbox-fingerprint sandbox-identity; do
		if [ -e "${CODE2OKF_STATE_DIR}/${host_only}" ]; then
...
if sudo -n true 2>/dev/null; then
	for rw in "${wiki_root}" "${CODE2OKF_STATE_DIR:-}/sessions"; do
		[ -d "${rw}" ] || continue
		scratch="$(mktemp -d)"
...
# Context7 native Pi package (kits/code2okf/spec.yaml setup.install +
...
# Credentials (kits/code2okf/spec.yaml). Automates the manual check in the README.
