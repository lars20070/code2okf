"""Tests for code2okf.sandbox — the one seam onto `sbx`, against FakeSbx."""
...
from code2okf import sandbox, workbench
...
def test_exists_before_and_after_create(fake_sbx):
    assert sandbox.exists("code2okf") is False
    sandbox.create("code2okf", Path("/kit"), [], {})
    assert sandbox.exists("code2okf") is True


def test_create_returns_a_token_identity_reads_back(fake_sbx):
    token = sandbox.create("code2okf", Path("/kit"), [], {})
    assert token
    assert sandbox.identity("code2okf") == token
...
def test_create_mounts_and_env_reach_the_argv(fake_sbx):
    ...
    sandbox.create("code2okf", Path("/kit"), mounts, {"CODE2OKF_STATE_DIR": "/state"})
...
def test_create_raises_on_sbx_run_failure(fake_sbx):
    fake_sbx.run_fail_names.add("code2okf")
    with pytest.raises(sandbox.SandboxError, match="code2okf"):
        sandbox.create("code2okf", Path("/kit"), [], {})
...
def test_create_removes_the_sandbox_when_the_token_write_fails(fake_sbx):
    ...
    """
    fake_sbx.token_write_fail_names.add("code2okf")
...
def test_probe_reflects_configured_state(fake_sbx):
    ...
    assert sandbox.probe("code2okf", Path("/work/okf")) is True
    fake_sbx.probe_ok = False
    assert sandbox.probe("code2okf", Path("/work/okf")) is False


def test_key_is_proxy_managed(fake_sbx):
    sandbox.create("code2okf", Path("/kit"), [], {})
    fake_sbx.openrouter_key = "proxy-managed"
    assert sandbox.key_is_proxy_managed("code2okf") is True
    fake_sbx.openrouter_key = "sk-literal-value"
    assert sandbox.key_is_proxy_managed("code2okf") is False


def test_exec_capture_runs_through_the_seam(fake_sbx):
    sandbox.create("code2okf", Path("/kit"), [], {})
    fake_sbx.queue_hash("abc123456789")
    result = sandbox.exec_capture("code2okf", ["merkleokf", "--nolog", "-L", "0", "/work/okf"])
...
def test_exec_stream_yields_lines_then_returncode(fake_sbx):
    ...
    stream = sandbox.exec_stream("code2okf", ["pi", "--mode", "json", "compile it"])
    lines = list(stream)
...
# --- python -m code2okf.sandbox (maintainers) ---------------------------------
...
def test_ensure_default_sandbox_respects_the_run_lock(fake_sbx, isolated_state, capsys):
    ...
    assert "another code2okf run" in capsys.readouterr().err
