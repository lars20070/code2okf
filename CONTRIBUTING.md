...
make validate            # check kits/code2okf/spec.yaml against the Sandbox Kit schema
...
make test-code2okf       # pytest, the code2okf driver suite
make test                # all of the above plus test-sandbox
make install             # install code2okf onto PATH (uv tool)
...
```bash
uv run code2okf md/                       # from a clone, no install
code2okf -o wikis/handbook docs/handbook/ # once `make install` has run
```
...
`test-shell`, `test-web2md`, `test-clis`, `test-code2okf`, `build-package`, and
`validate-kit`. Each one reuses the matching make target, so a green
`make lint && make validate && make test-shell && make test-web2md &&
make test-clis && make test-code2okf && make dist` locally means a green build.

## Validate the kit spec before you finish

Touch anything under `kits/code2okf/` or `scripts/*.sh` and run `make validate`
...
every tool `kits/code2okf/spec.yaml` installs, the agent config copied in from
`kits/code2okf/files/`, and a proxy-managed key? It needs an `sbx login` session.
...
the sandbox you have, which may be older than your last `kits/code2okf/` edit. To
check the current kit from scratch, throw the sandbox away first with
`sbx rm --force code2okf`; building the next one takes minutes.

## Working inside the sandbox

Once a sandbox exists — `code2okf` builds one on first use, or
`uv run python -m code2okf.sandbox` makes one without compiling anything — these
...
```bash
sbx exec -it code2okf -- bash   # interactive shell at the wiki root
sbx exec -it code2okf -- pi     # interactive Pi in the same sandbox
```

Once a sandbox exists, this should print `proxy-managed` rather than your key:

```bash
sbx exec code2okf -- sh -lc 'echo "$OPENROUTER_API_KEY"'
```

## Python layout

Python tooling is thin. The repository root *is* a project — it holds the
`code2okf` command itself, because the tool is named after the repository and a
...
The instructions come in two parts. `kits/code2okf/files/home/.pi/agent/AGENTS.md`
...
The kit is `kits/code2okf/`, and the config it carries lives in
`kits/code2okf/files/home/.pi/agent/`. That config is copied into the sandbox when
the kit is built, not mounted, so an edit reaches Pi on the next fresh sandbox —
which `code2okf` builds by itself once the kit's hash no longer matches what the
running one was built from, or immediately on `--fresh`.
[The kit guide](kits/code2okf/README.md) covers the model and provider settings.
...
The gate is `kits/code2okf/files/home/.pi/agent/skills/compile-okf/scripts/check-okf.sh`,
...
Release that does not exist yet, and a re-run is safe end to end: the build
job's artifact upload overwrites rather than colliding with the previous run's,
and PyPI treats an upload of a byte-identical file as idempotent.
...
5. Tag the merged commit, not whatever is checked out. After a squash or merge
   commit, a local branch `HEAD` is not the commit that landed, and tagging it
   would publish a tree that was never reviewed on `main`:
   ...
   git tag -a vX.Y.Z -m 'code2okf vX.Y.Z'
   git show --stat vX.Y.Z      # confirm this is the commit you mean to release
