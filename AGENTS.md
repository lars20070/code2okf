...
> from `kits/code2okf/files/home/.pi/agent/AGENTS.md`) — nothing here changes that.
...
code2okf compiles Markdown into an OKF wiki with the Pi coding agent: one Pi run
per source document, folded into the wiki. The `code2okf` command is the host
driver, at the repository root (`pyproject.toml`, `src/code2okf/`); it takes any
files or folders and writes to any `-o` directory, staging both through a fixed
workbench under `$XDG_STATE_HOME/code2okf` so one sandbox serves every run. In
...
Pi runs in one runtime: the Docker Sandbox (sbx) kit rooted at `kits/code2okf/`.
Its spec is `kits/code2okf/spec.yaml` and its Pi config (`AGENTS.md`,
`settings.json`, `models.json`, `skills/`) lives in
`kits/code2okf/files/home/.pi/agent/`. The agent has `bash`, so it lints
...
only land in a fresh sandbox — which `code2okf` builds when the kit's hash stops
...
make test-code2okf       # pytest, the code2okf driver suite (offline, fake sbx)
make test                # all of the above plus test-sandbox
make install             # uv tool install code2okf onto PATH
make install-clis        # uv tool install the four host CLIs onto PATH
make dist                # build wheel + sdist and smoke-test the artifact
make test-sandbox        # check the sandbox delivers what kits/code2okf/spec.yaml promises
...
```bash
uv run code2okf md/                          # compile from a clone, no install
uv run code2okf -o wikis/other docs/other/   # any input folder, any output folder
uv run code2okf --dry-run md/                # resolve and print; no sandbox, nothing paid
uv run code2okf -n 20 md/                    # raise the per-document iteration cap
uv run python -m code2okf.sandbox            # ensure the sandbox exists, compile nothing
sbx exec -it code2okf -- bash                # shell into it; `-- pi` for interactive Pi
sbx rm --force code2okf                      # discard it; the next run rebuilds
...
ours and still matches. `code2okf` takes `OPENROUTER_API_KEY` from `sbx secret`,
not from your shell (see the README for the two-step setup). Anything that
touches a sandbox — `code2okf`, `make test-sandbox`, `sbx exec` — needs an active
...
Whenever you change anything under `kits/code2okf/` or `scripts/*.sh`, you MUST validate the Pi
...
This checks `kits/code2okf/spec.yaml` against the current Sandbox Kit schema (a
...
installs or what it carries in `kits/code2okf/files/`, also run `sbx rm --force code2okf &&
